...
var tag = core.getInput('tag-name', requiredArgOptions);
var repoInput = core.getInput('repository', { trimWhitespace: true });
var owner = repoInput ? repoInput.split('/')[0] : github.context.repo.owner;
var repo = repoInput ? repoInput.split('/')[1] : github.context.repo.repo;
var octokit = github.getOctokit(token);
function writeToFile(assetName2, download) {
  try {
    core.info(`
Writing download to file "${assetName2}"...`);
    const bufferFromArray = Buffer.from(download);
    const downloadFilePath = `${process.cwd()}/${assetName2}`;
    ...
    core.setFailed(`Writing download to file "${assetName2}" failed: ${error.message}`);
  }
}
async function downloadAsset(owner2, repo2, assetId) {
  core.info(`
...
function getAssetFromRelease(assetsList, assetName2) {
  core.info(`
Looking for asset with name "${assetName2}"...`);
  if (!assetsList || assetsList.length === 0) {
    core.setOutput('error-condition', 'Release does not have assets');
    core.setFailed(`The release does not contain any assets.`);
    return;
  }
  const asset = assetsList.find(({ name }) => name === assetName2);
  if (!asset) {
    core.setOutput('error-condition', 'No assets match the name');
    core.setFailed(`No asset found with name "${assetName2}"`);
  } else {
    core.info(`Found asset with name "${assetName2}"`);
  }
  return asset;
}
async function getRelease(tag2) {
  core.info(`
Looking for release with tag "${tag2}"...`);
  let release;
  await octokit.rest.repos
    .getReleaseByTag({
      owner,
    ...
    .then(response => {
      core.info(`Found release with tag "${tag2}"`);
      release = response.data;
    })
    .catch(error => {
      if (error.status === 404) {
        core.setOutput('error-condition', 'Release with tag does not exist');
        core.setFailed(`The release with tag "${tag2}" does not exist`);
      } else {
        core.setOutput('error-condition', 'Unknown error getting release');
        core.setFailed(`An error occurred getting release with tag "${tag2}": ${error.message}`);
      }
    });
  return release;
}
